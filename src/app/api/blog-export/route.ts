...
import { blogExports, subscriptions } from '@/lib/db/schema';
...
// Function to estimate tokens (same as chat API)
...
<body>
  ...
  <div class="generated-by">
    <p>Generated by <strong>EscapeArtist AI</strong> - Advanced AI Search Engine</p>
...
export async function POST(req: NextRequest) {
  try {
    const { content, userQuestion, chatModel, sources, chatId, messageId, userId, guestId } = await req.json() as BlogExportRequest;

    // Get user's preferred model from request or use default
    ...
    // Check if blog export is allowed for this user
    if (userId) {
      ...
      // Blog export is only available for pro users
      if (plan === 'free') {
        return NextResponse.json(
          {
            error: 'Blog export not available',
            details: {
              message: 'Blog export is only available for Pro subscribers. Upgrade to Pro to access this feature.',
              feature: 'Blog Export',
              plan: 'free',
              requiresPlan: 'pro',
      ...
      // Guest users cannot access blog export
      return NextResponse.json(
        {
          error: 'Authentication required',
          details: {
            message: 'Blog export requires a Pro subscription. Please sign up and upgrade to Pro to access this feature.',
            feature: 'Blog Export',
            requiresAuth: true,
            requiresPlan: 'pro',
          },
    ...
    // Check usage limits for authenticated users before processing
    if (userId) {
      // Estimate tokens for the blog export (typically larger than chat)
      const estimatedInputTokens = estimateTokens(content) + 2000; // Blog prompt is large
      const estimatedOutputTokens = 3000; // Blog posts are typically long
      const estimatedTotalTokens = estimatedInputTokens + estimatedOutputTokens;
      
      // Use a default model for limit checking if not specified
      const modelForLimitCheck = selectedModel.name || 'gpt_4o';
      
      try {
        const usageCheck = await checkUsageLimit(userId, modelForLimitCheck);
          ...
          return NextResponse.json(
            {
              ...
              details: {
                message: `You have exceeded your ${modelForLimitCheck.replace(/_/g, '-')} usage limit for this month. Blog export requires significant token usage.`,
                currentUsage: usageCheck.currentUsage,
                limit: usageCheck.limit,
                remaining: usageCheck.remaining,
                model: modelForLimitCheck,
                feature: 'Blog Export',
              },
            },
            { status: 429 }
        ...
        if (usageCheck.remaining < estimatedTotalTokens) {
          return NextResponse.json(
            {
              error: 'Insufficient tokens for blog export',
              details: {
                message: `Blog export requires approximately ${estimatedTotalTokens} tokens, but you only have ${usageCheck.remaining} remaining for ${modelForLimitCheck.replace(/_/g, '-')}.`,
        ...
        console.error('Blog export usage limit check failed:', error);
    ...
    // Respect user's model selection first, with fallback priority order
    let selectedModelInfo = null;
    
    // First: Use user's preferred model if specified and available
    if (selectedModel && chatModels[selectedModel.provider] && chatModels[selectedModel.provider][selectedModel.name]) {
      selectedModelInfo = selectedModel;
      console.log(`Using user's selected model: ${selectedModel.provider}/${selectedModel.name}`);
    } else {
      // Fallback priority order if user's model is not available
      const modelPriority = [
        ...
        { provider: 'openai', models: ['gpt-4.1', 'gpt-4.1-mini', 'gpt-4.1-nano'] },
        // Gemini 2.5 Pro (third fallback)
        { provider: 'gemini', models: ['gemini-2.5-pro-exp-03-25', 'gemini-2.0-flash'] },
        // GPT-4o-mini (fourth fallback)
        { provider: 'openai', models: ['gpt-4o-mini'] },
        // Other fallback models
        { provider: 'openai', models: ['gpt-4o', 'gpt-4-turbo', 'gpt-4'] },
        { provider: 'anthropic', models: ['claude-3-5-sonnet-20241022', 'claude-3-sonnet-20240229'] },
        ...
        if (chatModels[provider]) {
          for (const modelName of models) {
            if (chatModels[provider][modelName]) {
              ...
              console.log(`Fallback model selected for blog generation: ${provider}/${modelName}`);
              break;
            }
          }
          if (selectedModelInfo) break;
        }
      }
    }
    ...
    if (!selectedModelInfo) {
      ...
      if (firstProvider && Object.keys(chatModels[firstProvider]).length > 0) {
        ...
        console.log(`Emergency fallback to first available model: ${firstProvider}/${firstModel}`);
      } else {
        return NextResponse.json(
    ...
    const model = chatModels[selectedModelInfo.provider][selectedModelInfo.name].model;
    ...
    if (userId && response.content) {
      try {
        // Calculate tokens: input prompt + output response
        const inputTokens = estimateTokens(fullPrompt);
        const outputTokens = estimateTokens(response.content.toString());
        const totalTokens = inputTokens + outputTokens;
        
        // Use the original model name - normalization is handled in trackUsage
        const modelNameForTracking = selectedModelInfo.name || 'unknown';
        
        console.log(`Tracking blog export usage: ${modelNameForTracking}, User: ${userId}, Tokens: ${totalTokens}, Provider: ${selectedModelInfo.provider}`);
        
        await trackUsage(userId, modelNameForTracking, totalTokens);
      } catch (error) {
        console.error('Failed to track blog export usage:', error);
      }
    }
